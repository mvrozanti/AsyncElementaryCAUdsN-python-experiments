#!/usr/bin/env python
# Problema: dentre os pares ⟨R,E⟩ selecionados, quais são candidatos para resolver o problema da paridade? 
# No caso de não haver nenhum, contar os scores para cada comprimento de reticulado. 
#
# Problem: among the selected ⟨R,E⟩ pairs, which of them are candidates to solve the parity problem? 
...
def lattice_solves_parity_problem(lattice):
    should_converge_to = lattice[0].count(1) % 2
    for space in lattice[1:]:
        if space.count(should_converge_to) == len(space):
            return True
    return False

def get_parity_problem_score(rule, scheme, n):
    if not n % 2:
        print(f'Parity problem does not support n={n}')
        sys.exit(1)
    t = T(n)
    score = 0
    for ic in gen_lattice_combo(n):
        lattice = list(run(rule, n, t, scheme, ic))
        score += int(lattice_solves_parity_problem(lattice))
    return score
    ...
    for rule,schemes in pairs.items():
        ...
        for scheme in schemes:
            if should_run_pair(scores_n, cur_n, rule, scheme):
                score_n_rule_scheme = get_parity_problem_score(rule, scheme, cur_n)
            else:
...
for n in scores_n:
    ...
    df = df.T
    df.to_csv(f'scores-PP-{n}x{T(n)}.csv')
