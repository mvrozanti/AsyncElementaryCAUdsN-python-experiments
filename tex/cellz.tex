...
\begin{itemize}
	\item Largura do espaço celular\\
          \tab N, como definido em \ref{referencial}. É o tamanho do espaço celular unidimensional.
...
O objetivo do presente trabalho foi identificar e quantificar pares (R,E) em que há conservabilidade nas evoluções de ACEs com atualização por prioridade da vizinhança.
...
A motivação para o presente trabalho é explorar em que condições os ACEs são conservativos com o propósito de enumerar evoluções conservativas que podem ser usadas para <+>.

\subsection{DELIMITAÇÃO DO ESTUDO} \label{delimitacao}
A pesquisa se limita a ACEs com o tipo de atualização evidenciado no item \ref{objetopesq}, com largura de reticulados testados até N=15. Os esquemas de atualização foram filtrados a fim de se eliminar esquemas equivalentes, já que produzem evoluções idênticas, resultando em 4683 esquemas. 
...
\tab Os ACEs são comumente representados por seu respectivo ``Wolfram code'' \cite{wolfram1983cellular}, uma nomenclatura que dá a cada regra um identificador no intervalo \(\left[0, 255\right]\):
...
\begin{figure}[!htbp]
\centerline{\includegraphics[width=10cm]{imgs/256ecas.png}}
...
% \begin{figure}[!htbp]
% \centerline{\includegraphics[width=10cm]{imgs/rule-transitions.png}}
...
\tab Esta forma de atualizar assincronamente o espaço celular pode ser descrita como um par \textit{(R,E)}, onde \textit{R} é a regra e \textit{E} é um esquema de prioridade. Eliminados os esquemas equivalentes, para ACEs, é possível gerar 1.194.165 combinações de pares \textit{(R,E)}. 
...
Como é mencionado no item \ref{contex}, ACs conservativos podem ser usados para modelar ambientes em que há conservabilidade, como trânsito automobilístico, simulação de partículas, entre outros. A regra 184 \ref{184} é uma regra que, com execução síncrona e (como este trabalho apresenta) assíncrona, gera evoluções conservativas pela definição anterior de conservabilidade.
...
\tab Este alicerce teórico foi obtido através de autores como: Wolfram; de Oliveira;
...
\tab Esta é uma pesquisa exploratória, dada a natureza desconhecida das propriedades de ACEs com o tipo de atualização mencionado em \ref{objetopesq}. Baseada em medidas objetivas e dados verificáveis, esta pesquisa foi voltada para encontrar caminhos, formas, maneiras e procedimentos para atingir um determinado fim, buscando definir um processo ou uma ferramenta que leve à solução do problema proposto (\ref{problema}).  Quanto aos meios, foram utilizados os recursos mencionados na Bibliografia.
...
\begin{table}[h]
    ...
    \centerline{
        \begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|l|l|}
          ...
          \hline
          Revisão bibliográfica                                & \cellcolor{yellow}            & \cellcolor{yellow} &                    &                    &                    &                    &                    & \cellcolor{yellow} &                    &                    &                    & \\
          \hline
          Fundamentação teórica     &                               & \cellcolor{yellow} & \cellcolor{yellow} &                    &                    &                    &                    &                    &                    &                    &                    & \\
          \hline
          Desenvolvimento e implementação Python                           &                               & \cellcolor{yellow} & \cellcolor{yellow} &                    &                    &                    &                    &                    &                    &                    &                    & \\
          \hline
          Conclusão e documentação final da pesquisa   &                               &                    &                    & \cellcolor{yellow} & \cellcolor{yellow} &                    &                    &                    &                    &                    &                    & \\
          \hline
          Simulação &                               &                    &                    & \cellcolor{yellow} & \cellcolor{yellow} &                    &                    &                    &                    &                    &                    & \\
          \hline
          Análise das simulações                     &                               &                    &                    &                    &                    & \cellcolor{yellow} & \cellcolor{yellow} &                    &                    &                    &                    & \\
          \hline
          Conclusão e documentação final da pesquisa                                  &                               &                    &                    &                    &                    & \cellcolor{yellow} & \cellcolor{yellow} & \cellcolor{yellow} & \cellcolor{yellow} & \cellcolor{yellow} &    \cellcolor{yellow}                & \cellcolor{yellow}\\
          \hline
    \end{tabular}
  }
\end{table}
