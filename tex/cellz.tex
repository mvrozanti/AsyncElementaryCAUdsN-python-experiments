...
\newcommand{\orientador}{Prof. Pedro Paulo Balbi}
...
\newcommand{\titulo}{
  ...
  \\~\\
  \normalfont{\normalsize{Marcelo Vironda Rozanti\\Felipe Stefanelli de Aguiar Silva\\Pedro Paulo Balbi (Orientador) \\ Faculdade de Computação e Informática - Universidade Presbiteriana Mackenzie \\São Paulo, SP -- Brasil}}\\
...
\begin{abstract}
  Cellular automata are discrete computational systems that have proved useful as general models of complexity and representation of systems' dynamics in a variety of scientific fields. These systems compute essentially by means of local functions and can even display universal computability. Here we report on the exploration of a fundamental subset of them, the elementary cellular automata, with a specific kind of asynchronous update scheme based on the update priority of the neighbourhood configurations that define the local function. We study the effect of this kind of update on those rules, in terms of their becoming number conserving, as well as solving the decision problems of density and parity classification. 
\end{abstract}
...
\tab Desde então, múltiplos estudos foram realizados por cientistas ao redor do mundo, destacando os trabalhos feitos por Stephen Wolfram na década de 1980, culminando na publicação do livro A New Kind of Science em que Wolfram apresenta uma coleção de resultados a respeito de ACs.  

\tab Tratamos aqui de uma pesquisa exploratória, envolvendo  Autômatos Celulares Elementares com um tipo especíifico de atualização assíincrona, conforme se apresentaría na Subseção \ref{udsn}. A seguir, são apresentados conceitos e experimentos relacionados a Autômatos Celulares Elementares assícronos.
...
% Vários tipos de atualização assícrona foram descritos por autores diferentes, de formas diferentes.  
...
% \tab Há uma infinidade de ACs passíveis de construção para modelagem ou solução de problemas. Um deles é a modelagem de tráfego de veículos, como é notado por \cite[p. 307]{rosenblueth2011model}.'
%
...
\begin{itemize}
  ...
  \item Definir e identificar a propriedade e problemas estudados;
\end{itemize}

\section{Referencial Teórico} 
\label{referencial}
Em relação ao referencial teórico necessíario, foi de suma importância entender os seguintes tópicos:

\subsection{Autômatos Celulares} \label{ref_ac}
De acordo com \cite{wolfram1983cellular}, ``sistemas matemáticos de simples construção são capazes de comportamento altamente complexo com muitas características''. ACs são um exemplo desse tipo de sistemas. ACs são compostos por células, cada uma com um estado. Eles podem ter \textit{d} dimensões e \textit{q} estados finitos, os quais devem ser estipulados previamente e são necessariamente computados através de funções locais, isto é, que atuam considerando a vizinhança das células.

\tab Wolfram afirma ainda que, autômatos celulares são simples idealizações matemíaticas de sistemas naturais. Eles consistem de um reticulado de com posições discretas idênticas, cada uma assumindo um conjunto finito de valores inteiros. Os valores dos locais evoluem em fases temporais, ou iterações, discretas de acordo com regras determiníisticas que especificam o valor de cada cíelula em termos dos valores das cíelulas adjacentes. Todo AC depende de uma \textit{configuração inicial}, que íe um reticulado de necessariamente \textit{d}-dimensções com cada cíelula com um estado pertencendo ao conjunto de \textit{q} estados.

\subsection{Autômatos Celulares Elementares} 
\label{ref_ace}

ACEs formam a família de ACs com \textit{d}=1, \textit{q}=2 e \textit{r}=1 (i.e., raio unitíario), ou seja, são unidimensionais, com células binárias e vizinhanças definidas como 3 células contíguas. O reticulado é tem comprimento \textit{n}, isto é, tem \textit{n} células.
...
\tab Uma definição direta das regras de transição consiste na enumeração de todos os estados possíveis da príopria cíelula, das cíelulas vizinhas e do príoximo estado, contendo a informação necessíaria para computar o estado futuro das cíelulas em função de sua vizinhança e, consequentemente, da evolução como um todo. A regra do AC corresponde a uma função local, formada por transições de estado. A ação dela sobre cada cíelula de uma configuração pode ser vista como resultado da ação de uma função global sobre o reticulado, que íe distinta da função local.
...
\tab Ou seja, uma evolução é dita conservativa caso, para qualquer configuração inicial, a soma dos estados presentes em qualquer configuração é sempre a mesma para todas as iterações. Como é mencionado na Subseção \ref{contex}, ACs conservativos podem ser usados para modelar ambientes em que há conservabilidade, como trânsito automobilístico, simulação de partículas, entre outros. A regra 184 é uma regra que, em execução síncrona, gera evoluções conservativas pela definição anterior de conservabilidade. 
...
\begin{figure}[h]
  \begin{minipage}[htb]{.5\linewidth}
    \centering
    \fbox{\includegraphics[width=0.1\linewidth]{imgs/184-5x33-11111111-large.png}}
